...
CMD ["sh", "-c", "echo '--- Starting migrate ---' && python manage.py migrate --noinput && echo '--- Migrate done, starting gunicorn ---' && gunicorn config.wsgi:application --bind 0.0.0.0:7860 --workers 2 --timeout 120 --access-logfile - --error-logfile - --log-level info"]