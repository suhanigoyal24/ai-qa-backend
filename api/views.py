...
class ListFilesView(APIView):
    """List files belonging ONLY to the logged-in user"""
    permission_classes = [IsAuthenticated]

    def get(self, request):
        try:
            files = UploadedFile.objects.filter(uploaded_by=request.user).order_by('-created_at')
...
class SummarizeView(APIView):
    """Generate AI summary for a processed file (only if it belongs to the user)"""
    permission_classes = [IsAuthenticated]
...
class ChatView(APIView):
    """Ask questions about an uploaded file (only if it belongs to the user)"""
    permission_classes = [IsAuthenticated]
